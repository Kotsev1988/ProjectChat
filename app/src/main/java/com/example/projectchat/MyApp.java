...
class MyCall extends Call
{
    ...
    public void onCallMediaState(OnCallMediaStateParam prm)
    {
        ...
        for (int i = 0; i < cmiv.size(); i++) {
            ...
            Log.d("CallCmi", ""+cmi.getStatus());
            Log.d("VideoTAG", String.format("onCallMediaState: media - %d, type - %d, status - %d, incomingWindowId - %d",
                    i, cmi.getType(), cmi.getStatus(), cmi.getVideoIncomingWindowId()));
            ...
            if (cmi.getType() == pjmedia_event_type.PJMEDIA_EVENT_VID_DEV_ERROR){
                Log.d("VideoError", "VidErr");
            }
        }
...
class MyApp extends pjsua2 {
    ...
    private MyAccountConfig accCfgs =new MyAccountConfig();
                     ...
                     boolean own_worker_thread)
    {
     ...
     /*  try {
       ...
       // for (int i = 0; i < accCfgs.size(); i++) {
            MyAccountConfig my_cfg = accCfgs;
